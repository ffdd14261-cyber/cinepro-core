...
export class NxshaProvider extends BaseProvider {
    readonly id = 'nxsha';
    readonly name = 'Nxsha';
    readonly enabled = true;
    readonly BASE_URL = 'https://web.nxsha.app/';
    readonly HEADERS = {
        'User-Agent':
    ...
    private emptyResult(
        ...
        return {
            ...
            diagnostics: [
                {
                    code: 'PROVIDER_ERROR',
                    message: `${this.name}: ${message}. Note that Nxsha blocks all kinds of VPN IPs, so if you are using one, try disabling it and see if that helps.`,
                    field: '',
                    severity: 'error'
                }
            ]
